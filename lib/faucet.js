...
utils.ready().then(() => {
    ...
    // wait for sodium to initialize
    client_1.initializeWallet(stateWallet => ({
    ...
    // activate wallet
    client_1.activateWallet(stateWallet => ({
        secret: stateWallet.wallet.secret
    })), 
    // wait for transaction to be confirmed
    client_1.confirmOperation(stateWallet => ({
    ...
    operators_1.catchError((error) => {
        return error.response && error.response[0].id === 'proto.alpha.operation.invalid_activation' ?
            rxjs_1.of({ wallet: error.state.wallet }) :
            rxjs_1.throwError(error);
    }), 
    // get wallet info
    client_1.getWallet(), operators_1.tap((stateWallet) => console.log('[+] getWallet: balance', (stateWallet.getWallet.balance / 1000000))), 
    ...
    operators_1.flatMap(stateWallet => (stateWallet.getWallet.balance / 1000000) > 1 ?
        ...
        // send xtz
        client_1.transaction(stateWallet => ({
