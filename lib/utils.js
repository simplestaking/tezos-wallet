...
export const signOperationTrezor = (state) => {
        ...
        if (operation.kind === 'transaction') {
            ...
            if (operation.parameters) {
                message = Object.assign({}, message, { operation: Object.assign({}, message.operation, { 
                        ...
                        transaction: Object.assign({}, message.operation.transaction, { parameters: sodium.from_hex(state.operation.slice(state.operation.indexOf('000000d'), state.operation.length)) }) }) });
            }
        }
    ...
    return of([state]).pipe(tap(state => console.warn('[[TrezorConnect]][signOperationTrezor] message', JSON.stringify(message))), 
    // wait for resopnse from Trezor
    flatMap(state => window.TrezorConnect.tezosSignTransaction(message)), tap((response) => { console.warn('[TrezorConnect][tezosSignTransaction] reposne', response.payload); }), map(response => (Object.assign({}, state, { signOperation: {
