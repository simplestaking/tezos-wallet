...
export const signOperationTrezor = (state) => {
    ...
    return of(TrezorConnect.tezosSignTx(message)).pipe(map(response => (Object.assign({}, state, { signature: response.payload.signature, signedOperationContents: response.payload.sig_op_contents, operationHash: response.payload.operation_hash }))), catchError(error => {
