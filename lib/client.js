...
// inject operation
flatMap((state) => rpc('/injection/operation', '"' + state.signedOperationContents + '"').pipe(
// tap((response: any) => console.log("[+] operation: inject ", response)),
map(response => (Object.assign({}, state, { injectionOperation: response }))), tap((state) => console.log("[+] operation: http://zeronet.tzscan.io/" + state.injectionOperation)))));
/**
 * Wait until operation is confirmed & moved from mempool to head
 */
...
flatMap((response) => {
    // console.log('[confirmOperation]', response, state)
    // debugger
