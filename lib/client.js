...
map(state => {
    ...
    if (state.transaction.parameters) {
        operations[operations.length - 1] = Object.assign({}, operations[operations.length - 1], { "parameters": state.transaction.parameters });
    }
...
/**
  * Activate wallet
  */
export const activateWallet = (fn) => (source) => source.pipe(map(state => (Object.assign({}, state, { 'activateWallet': fn(state) }))), 
// prepare config for operation
map(state => {
    const operations = [];
    operations.push({
...
// check for errors
flatMap(state => state.preapply[0].contents[0].metadata.operation_result && state.preapply[0].contents[0].metadata.operation_result.status === "failed" ?
 ...
 */
export const pendingOperation = (fn) => (source) => source.pipe(map(state => (Object.assign({}, state, { 'pendingOperation': fn(state) }))), 
