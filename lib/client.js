...
// create operation 
exports.operation(), operators_1.tap(function (state) { return console.log('[+] origination:  ', state); })); }; };
/**
...
operators_1.flatMap(function (state) {
    ...
    operators_1.map(function (response) { return (__assign({}, state, { operations: response })); }));
}), 
// inject operation
operators_1.flatMap(function (state) {
    return rpc_1.rpc('/injection/operation', '"' + state.signedOperationContents + '"').pipe(operators_1.tap(function (response) { return console.log("[+] operation: inject ", response); }), operators_1.map(function (response) { return (__assign({}, state, { injectionOperation: response })); }), operators_1.tap(function (state) { return console.log("[+] operation: http://zeronet.tzscan.io/" + state.injectionOperation); }));
})); }; };
/**
