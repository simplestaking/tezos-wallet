 ...
 */
export const confirmOperation = (fn) => (source) => source.pipe(map(state => (Object.assign({}, state, { 'confirmOperation': (fn && typeof fn === 'function') ? fn(state) : state.confirmOperation }))), tap((state) => console.log('[-] pending: operation "' + state.confirmOperation.injectionOperation + '"', state)), 
 ...
 */
export const initializeWallet = (fn) => (source) => source.pipe(
// wait for sodium to initialize
