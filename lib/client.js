...
operators_1.flatMap(function (state) {
    ...
    // add counter to state 
    operators_1.map(function (response) { return (__assign({}, state, { manger: response.manager ? response.manager : undefined, key: response.key ? response.manager : undefined })); }));
})); }; };
/**
